...
<Note>
For optimal results with [word-level streaming](/build-with-cartesia/capability-guides/stream-inputs-using-continuations), we recommend using `max_buffer_delay_ms`. You can tune this parameter based on your latency and quality requirements: lower values optimize for responsiveness, while higher values produce better prosody.
</Note>
