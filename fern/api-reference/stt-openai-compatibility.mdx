...
Cartesia's Batch Speech-to-Text API provides compatibility with OpenAI's client libraries through the `/audio/transcriptions` endpoint. This allows for migration from OpenAI's Whisper API to Cartesia's STT service.
...
- **OpenAI Compatible**: `/audio/transcriptions` - OpenAI's existing endpoint for seamless migration
