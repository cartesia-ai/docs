...
Each HTTP request is treated as a separate context. Every new request counts toward your concurrency limit. Each unique <code>context_id</code> represents a context. Sending additional messages with the same <code>context_id</code> does not increase your concurrency usage. Messages within the same context are processed one at a time, not concurrently.
