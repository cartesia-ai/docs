...
- For WebSockets, a unique <code>context_id</code> defines a context—sending additional messages with the same <code>context_id</code> does not increase your concurrency usage. Messages within the same context are processed sequentially, not in parallel.
