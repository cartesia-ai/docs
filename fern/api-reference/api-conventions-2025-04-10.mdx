...
- While we will continue to support HTTP requests with `X-API-Key: <your_api_key>` in the HTTP headers, we recommend using `Authorization: <your_api_key>`.
- For client-side HTTP requests using Access Tokens, include `Authroization: Bearer <generated_access_token>` in the HTTP headers.
