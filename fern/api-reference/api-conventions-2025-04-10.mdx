...
- While we will continue to support HTTP requests with `X-API-Key: <your_api_key>` in the HTTP headers, we recommend using `Authorization: Bearer <your_api_key>`.
- For client-side HTTP requests using Access Tokens, include `Authorization: Bearer <access_token>` in the HTTP headers. Access Tokens are only accepted for `/tts` endpoints at this time.
