...
For server-side API connections, you can use your API key directly by authenticating your requests with `Authorization: Bearer <your_api_key>` in the headers.
Use your API key directly only in secure environments to prevent malicious actors from intercepting your key.

To make Cartesia API requests from your client app without exposing your API key, we have Access Tokens. After generating an Access Token, include `Authorization: Bearer <access_token>` in the HTTP headers of Cartesia API requests.
Visit [Access Token API Reference](/2025-04-16/api-reference/auth/access-token) for more info.
