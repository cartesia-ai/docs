...
if __name__ == "__main__":
    ...
    DATASET_ID = create_dataset("PVC demo dataset", "Samples for a Pro Voice Clone")
    ...
    for wav in Path("samples").glob("*.wav"):
        fid = upload_file(wav)
        FILE_IDS.append(fid)
        print("Uploaded", wav.name, "→", fid)

    # 3️⃣  Attach files to the dataset
    for fid in FILE_IDS:
        add_file_to_dataset(DATASET_ID, fid)
        print("Attached", fid, "to", DATASET_ID)
    ...
    FINE_TUNE_ID = create_fine_tune(
        ...
        name="PVC example",
        language="en",
        model_id="tts-2025-04-16",  # replace with current base model id
    )
    print("Started fine-tune:", FINE_TUNE_ID)
    ...
    checkpoints = list_checkpoints(FINE_TUNE_ID)
    print("Voices:")
