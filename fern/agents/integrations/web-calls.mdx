...
1. **Server → Client:** Receive an ack event confirming configuration.
1. **Bidirectional exchange:** The client and server exchange streaming audio and control events until either side closes the connection, or the inactivity timeout is fired.
1. **Close:** Either side ends the session with a standard WebSocket close frame.
