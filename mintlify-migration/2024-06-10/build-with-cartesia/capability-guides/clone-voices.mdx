...
Voice cloning is available through the [playground](https://play.cartesia.ai) and the [API](/2024-06-10/api-reference/voices/clone). Voice cloning has two modes: stability and similarity. High-similarity clones sound more like the source clip, but may reproduce background noise. Stability clones always sound like a studio recording, but may not sound as similar to the source clip.
