...
<Info>
This is a hands-on guide to input streaming using WebSocket contexts. For a conceptual overview of how input streaming works in Sonic, see the [input streaming guide](/2024-06-10/build-with-cartesia/capability-guides/stream-inputs-using-continuations).
</Info>
...
<Tip>
If [streaming in input tokens](/2024-06-10/build-with-cartesia/capability-guides/stream-inputs-using-continuations), we recommend using `max_buffer_delay_ms`, which sets the maximum time the model will buffer text before starting generation.
