...
description: Learn about managing multiple transcript generations with context flushing.
icon: cube
---
## Overview

When using [context IDs with the WebSocket API](/2024-11-13/api-reference/tts/working-with-web-sockets/contexts), all audio chunks for transcripts submitted to a single context share the same context ID. This makes it difficult to determine which audio chunks correspond to specific transcript submissions.
